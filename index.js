document.addEventListener("DOMContentLoaded", () => {
  ...
  peer.on("connection", (conn) => {
    ...
    const remotePeerName =
      conn.metadata?.name || `Peer_${conn.peer.substring(0, 4)}`;
    console.log(
      `Incoming connection from ${remotePeerName} (${conn.peer}) with metadata:`,
      conn.metadata
    );
    console.log(
      `Accepting data connection from ${remotePeerName} (${conn.peer})`
    );
  ...
  connectButton.addEventListener("click", () => {
    ...
    if (!remoteId)
      return displayPopup(
        "<i class='bi bi-info-circle-fill'></i> Please enter a peer ID."
      );
    if (connections[remoteId])
      return displayPopup(
        `<i class='bi bi-info-circle-fill'></i> Already connected to ${connections[remoteId].name} (${remoteId})`
      );
    if (remoteId === peer.id)
      return displayPopup(
        "<i class='bi bi-info-circle-fill'></i> You cannot connect to yourself."
      );
    ...
    conn.on("error", (err) => {
      ...
      displayPopup(
        `<i class='bi bi-info-circle-fill'></i> Failed to connect to ${remoteId}. Error: ${err.type}`
      );
      if (
        connectInput.value === remoteId ||
        connectButton.textContent === "Connecting..."
      ) {
        connectButton.disabled = false;
        connectButton.innerHTML = '<i class="bi bi-wifi"></i> Connect';
  ...
  function setupConnection(conn, initialRemotePeerName) {
    ...
    console.log(
      `Setting up data connection with ${initialRemotePeerName} (${remotePeerId})`
    );
    ...
    conn.on("open", () => {
      ...
      const currentName =
        connections[remotePeerId]?.name || initialRemotePeerName; // Use stored name if available
      console.log(
        `Data connection established with ${currentName} (${remotePeerId}). Sending/Receiving names.`
      );
      // Send our name to the remote peer
      try {
        console.log(
          `Sending name '${localPeerName}' to ${currentName} (${remotePeerId})`
        );
        conn.send({ type: "peer-name", name: localPeerName });
      } catch (error) {
        console.error(
          `Error sending name to ${currentName} (${remotePeerId}):`,
          error
        );
      }
      ...
      if (
        ...
        console.log(
          `Updating name for ${remotePeerId} from metadata: ${metadataName}`
        );
        connections[remotePeerId].name = metadataName;
  ...
  function sendFileChunked(file, conn, progressBar, progressElement) {
    ...
    function readSlice() {
      ...
      reader.onload = (evt) => {
        ...
        if (!connections[peerId] || !connections[peerId].conn?.open) {
          console.warn(
            `Connection to ${currentReceiverName} (${peerId}) lost just before sending chunk for ${file.name}. Aborting send.`
          );
        ...
        try {
          ...
          if (offset < file.size) {
            ...
            const finalReceiverName =
              connections[peerId]?.name || currentReceiverName; // Use last known as fallback
            console.log(
          ...
          console.error(
            `Error sending chunk for ${file.name} to ${currentReceiverName} (${peerId}):`,
            error
          );
  ...
  function handleData(data, conn) {
    ...
    let peerName =
      connections[peerId]?.name || `Peer_${peerId.substring(0, 4)}`; // Fallback name
    ...
    const safeFileName =
      data.fileName?.replace(/[^a-zA-Z0-9_-]/g, "-") || "unknownfile";
    ...
    if (
      ...
      console.log(
        `Received metadata for ${data.fileName} (${formatBytes(
          data.totalSize
        )}) from ${peerName} (${peerId})`
      );

      // Check if transfer already exists (e.g., duplicate metadata message)
      if (incomingTransfers[key]) {
        console.warn(
          `Received duplicate metadata for ongoing transfer: ${key} from ${peerName}. Ignoring.`
        );
        return;
      }
      ...
      if (existingLi) {
        console.warn(
          `UI element for ${key} already exists. Removing old element before starting new transfer.`
        );
        existingLi.remove();
      }
      ...
      const label = document.createTextNode(
        `Receiving ${data.fileName} from ${peerName}: `
      );
      const progress = document.createElement("progress");
      ...
      if (!transfer) {
        ...
        console.error(
          `Received file chunk for ${data.fileName} from ${peerName} (${peerId}) before metadata. Discarding chunk.`
        );
        const existingLi = document.getElementById(sanitizedKey);
        if (existingLi && !existingLi.querySelector("a")) {
          // If it's still showing progress/error
      ...
      const chunkBuffer =
        data.chunk instanceof ArrayBuffer
          ? data.chunk
          : new Uint8Array(data.chunk).buffer;
      ...
      if (data.isLast) {
        ...
        const finalPeerName =
          transfer.peerName || connections[peerId]?.name || peerName;
        if (transfer.receivedBytes === transfer.totalSize) {
          ...
          transfer.progressElement.append(
            a,
            document.createTextNode(` (from ${finalPeerName})`)
          );

          console.log(
            `Finished receiving ${data.fileName} from ${finalPeerName} (${peerId})`
          );
        } else {
          // Error: Received 'last chunk' flag but byte count doesn't match
          console.error(
            `File transfer incomplete for ${data.fileName} from ${finalPeerName} (${peerId}). Expected ${transfer.totalSize} bytes, received ${transfer.receivedBytes}`
          );
      ...
      if (connections[peerId]) {
        ...
        if (connections[peerId].name !== data.name) {
          ...
          console.log(
            `Updating UI for peer ${peerId} with new name: ${data.name}`
          );
          updatePeerList(); // Update the connected peers list display
          displayPopup(
            `<i class='bi bi-check-circle-fill'></i> Connected to ${data.name}`
          );
          // Update any ongoing transfer UI elements for this peer
          Object.values(incomingTransfers).forEach((transfer) => {
            if (
              transfer.peerName !== data.name &&
              transfer.progressElement.textContent.includes(peerName)
            ) {
          ...
          Object.values(sendProgressList.querySelectorAll("li")).forEach(
            (li) => {
              const parts = li.id.split("-"); // send-${peerId}-${safeFileName}
              if (
                parts.length >= 3 &&
                parts[1] === peerId &&
                li.textContent.includes(peerName)
              ) {
                if (
                  li.querySelector("progress") ||
                  li.textContent.includes("Sending") ||
                  li.textContent.includes("Sent")
                ) {
          ...
          console.log(
            `Received name ${data.name} for ${peerId}, but it's already set. No UI update needed.`
          );
        }
      } else {
        console.warn(
          `Received name for unknown/disconnected peer ${peerId}. Ignoring.`
        );
      }
    } else {
      // Handle other data types if needed
      console.log(
        `Received unknown data type from ${peerName} (${peerId}):`,
        data
      );
    }
  }
